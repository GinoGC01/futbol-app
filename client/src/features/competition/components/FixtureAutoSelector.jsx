...
export default function FixtureAutoSelector({ open, onClose, fase, equipos, ligaId, currentTemporada }) {
  ...
  const tieneGrupos = fase?.grupos?.length > 0

  useEffect(() => {
    if (open) {
      if (tieneGrupos) {
        const eqInGroups = fase.grupos.flatMap(g => g.grupo_equipo?.map(ge => ge.equipo_id) || [])
        setSelectedTeams(eqInGroups)
      } else {
  ...
  function handleGenerate() {
    if (!tieneGrupos && selectedTeams.length < 2) return toast.error('Seleccioná al menos 2 equipos')
    if (tieneGrupos && selectedTeams.length === 0) return toast.error('No hay equipos en los grupos')
    setConfirming(true)
  }
  ...
  // Round-robin stats
  let totalRoundsRR = 0
  let totalMatchesRR = 0
  let matchesPerRoundStr = ""

  if (tieneGrupos) {
    let maxRounds = 0
    let totalMatches = 0
    fase.grupos.forEach(g => {
      const gEqCount = g.grupo_equipo?.length || 0
      if (gEqCount > 0) {
        const roundsPerLegG = gEqCount % 2 === 0 ? gEqCount - 1 : gEqCount
        const roundsG = idaYVuelta ? roundsPerLegG * 2 : roundsPerLegG
        if (roundsG > maxRounds) maxRounds = roundsG
        const matchesPerRoundG = Math.floor(gEqCount / 2)
        totalMatches += roundsG * matchesPerRoundG
      }
    })
    totalRoundsRR = maxRounds
    totalMatchesRR = totalMatches
    matchesPerRoundStr = "Grupos"
  } else {
    const roundsPerLeg = n % 2 === 0 ? n - 1 : n
    totalRoundsRR = idaYVuelta ? roundsPerLeg * 2 : roundsPerLeg
    matchesPerRoundStr = Math.floor(n / 2)
    totalMatchesRR = totalRoundsRR * matchesPerRoundStr
  }
        ...
        <div className="space-y-5 animate-fade-in">
            ...
            {isKnockout ? (
              ...
              <>
                <div className="grid grid-cols-2 gap-2">
                  {[{l:'Equipos',v:n},{l:'Jornadas',v:totalRoundsRR},{l:'Part/Jornada',v:matchesPerRoundStr},{l:'Total',v:totalMatchesRR}].map(s=>(
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {tieneGrupos && <span className="text-[9px] font-black text-primary bg-primary/10 px-2 py-1 rounded-md border border-primary/20 uppercase tracking-widest">Fase con Grupos</span>}
                  {!tieneGrupos && n % 2 !== 0 && <span className="text-[9px] font-black text-warning bg-warning/10 px-2 py-1 rounded-md border border-warning/20 uppercase tracking-widest">{n} impares — rotación descanso</span>}
                </div>
              </>
            )}
          ...
          <div>
            <p className="text-[10px] font-bold text-text-dim uppercase tracking-wider mb-2">
              Equipos que participan ({selectedTeams?.length || 0}/{equipos?.length || 0})
              {tieneGrupos && " - Bloqueado por grupos"}
            </p>
                ...
                equipos.map(eq => {
                  ...
                  return (
                    <button key={eq.id} onClick={() => toggleTeam(eq.id)} type="button"
                      className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl border text-left transition-all ${isSelected ? 'bg-primary/5 border-primary/30 text-text-primary' : 'bg-bg-surface border-border-subtle text-text-dim opacity-60'} ${tieneGrupos ? 'cursor-default' : 'cursor-pointer'}`}>
          ...
          <Button onClick={handleGenerate} className="w-full gap-2 h-14 bg-primary text-bg-deep font-black uppercase italic tracking-wide shadow-2xl shadow-primary/20" disabled={(!tieneGrupos && selectedTeams.length < 2) || (tieneGrupos && selectedTeams.length === 0)}>
          ...
          <div>
            ...
            <p className="text-sm text-text-dim">
              {isKnockout
                ...
                : <>Se generará un fixture de <span className="font-bold text-text-primary">{totalMatchesRR} partidos</span> en <span className="font-bold text-text-primary">{totalRoundsRR} jornadas</span> para <span className="font-bold text-text-primary">{tieneGrupos ? `los ${fase.grupos.length} grupos` : `${n} equipos`}</span>.{idaYVuelta && <><br/><span className="text-secondary font-bold">Formato ida y vuelta</span> — la segunda rueda invierte la localía.</>}</>
              }
            </p>

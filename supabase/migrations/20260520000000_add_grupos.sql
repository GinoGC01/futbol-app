-- ============================================================
-- 0. RECREAR FUNCIÓN HELPER (fue borrada por 014_custom_auth.sql CASCADE)
-- ============================================================
CREATE OR REPLACE FUNCTION public.liga_pertenece_al_auth(p_liga_id UUID)
RETURNS BOOLEAN
...
AS $$
  SELECT EXISTS (
    SELECT 1 FROM liga
    WHERE id = p_liga_id
      AND organizador_id = (auth.jwt() ->> 'sub')::UUID
  );
$$;

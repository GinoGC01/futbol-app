...
-- Issues: Security Definer Views (CRITICAL) + Function Search Path Mutable
-- ===========================================

-- -----------------------------------------------
-- SECTION 1: Views — Switch to SECURITY INVOKER
-- -----------------------------------------------

CREATE OR REPLACE VIEW public.vista_tabla_posiciones
...
CREATE OR REPLACE VIEW public.vista_goleadores
WITH (security_invoker = true)
AS
SELECT
  j.id              AS jugador_id,
...
CREATE OR REPLACE VIEW public.vista_tarjetas
WITH (security_invoker = true)
AS
SELECT
  j.id              AS jugador_id,
...
CREATE OR REPLACE VIEW public.vista_fixture
WITH (security_invoker = true)
AS
SELECT
  p.id              AS partido_id,
...
CREATE OR REPLACE VIEW public.vista_pagos
WITH (security_invoker = true)
AS
SELECT
...
-- -----------------------------------------------
-- SECTION 2: Functions — Fix Search Path
-- -----------------------------------------------

ALTER FUNCTION public.liga_pertenece_al_auth(p_liga_id UUID) SET search_path = public;
ALTER FUNCTION public.alerta_equipos_incompletos(p_liga_id UUID) SET search_path = public;
ALTER FUNCTION public.alerta_pagos_vencidos(p_liga_id UUID) SET search_path = public;
ALTER FUNCTION public.alerta_equipos_penalizados(p_liga_id UUID) SET search_path = public;
ALTER FUNCTION public.alerta_jugadores_penalizados(p_liga_id UUID) SET search_path = public;
ALTER FUNCTION public.evaluar_alertas() SET search_path = public;
ALTER FUNCTION public.alerta_pilar_temporadas(p_liga_id UUID) SET search_path = public;
ALTER FUNCTION public.alerta_pilar_cupos(p_liga_id UUID) SET search_path = public;
ALTER FUNCTION public.alerta_pilar_sanciones(p_liga_id UUID) SET search_path = public;
